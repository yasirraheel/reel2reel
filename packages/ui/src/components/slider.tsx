...
>(({ className, ...props }, ref) => (
  ...
  >
    ...
    {Array.from({ length: (Array.isArray(props.value) ? props.value.length : (Array.isArray(props.defaultValue) ? props.defaultValue.length : 1)) }).map((_, i) => (
      <SliderPrimitive.Thumb key={i} className="block h-2.5 w-2.5 rounded-full bg-white shadow-sm ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50" />
    ))}
  </SliderPrimitive.Root>
))
